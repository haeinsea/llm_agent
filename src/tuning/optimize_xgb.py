...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="search_xgb.yaml", help="Search config file name under configs/.")
    parser.add_argument("--output-prefix", default="xgb", help="Prefix for saved trial/best files.")
    parser.add_argument("--output-dir", default=None, help="Optional alternative output directory.")
    return parser.parse_args()


def run_search(*, config_name: str = "search_xgb.yaml", output_prefix: str = "xgb", output_dir: str | None = None) -> tuple[pd.DataFrame, dict]:
    print("\n" + "=" * 80, flush=True)
    ...
    print(f"[optimize_xgb] score_weights={weights}", flush=True)
