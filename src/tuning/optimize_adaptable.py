...
from src.models.temporal_sota import infer_tta_online_probs
...
def _trial_metrics(
    ...
    *,
    ...
    infer_batch_size = int(params.get("inference_batch_size", 512))
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        default="search_adaptable_add_figure_auc.yaml",
        help="Search config file name under configs/.",
    )
    parser.add_argument("--output-prefix", default="adaptable", help="Prefix for saved trial/best files.")
...
def run_search(
    *,
    config_name: str = "search_adaptable_add_figure_auc.yaml",
    output_prefix: str = "adaptable",
    output_dir: str | None = None,
) -> tuple[pd.DataFrame, dict]:
    print("\n" + "=" * 80, flush=True)
    print("[START] optimize_adaptable", flush=True)
    print("  objective : weighted validation objective with TTA inference", flush=True)
    ...
    print(f"[optimize_adaptable] train_windows={len(train_df):,} val_windows={len(val_df):,} seeds={seeds} trials={len(trial_space):,}", flush=True)
    print(f"[optimize_adaptable] score_weights={weights}", flush=True)

    for idx, params in enumerate(trial_space, start=1):
        print(f"[optimize_adaptable] trial {idx}/{len(trial_space)} params={params}", flush=True)
        seed_rows = []
        for seed in seeds:
            seed_rows.append(
                _trial_metrics(
    ...
    print("[DONE] optimize_adaptable", flush=True)
